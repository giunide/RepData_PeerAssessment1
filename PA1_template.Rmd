...
```
The mean and the median of the total number of steps taken per day correspond to `r as.integer(mean(dsteps, na.rm = TRUE))` and `r as.integer(median(dsteps, na.rm = TRUE))`, respectively
...
```
The 5 minutes interval witch contains the maximum number of steps is: `r avg_steps$interval[max_step]`.
...
sum(na)
```
The total number of missing values in the variable steps is: `r sum(na)`.
...
median(dsteps.new)
```
Now we can see there's an increase in the frequency of the steps if we fill the missing values, an we can confirm this by observing the new values of the mean (`r as.integer(mean(dsteps.new))`) and of the median (`r as.integer(median(dsteps.new))`).
